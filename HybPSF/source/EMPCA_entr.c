...
void EMPCA_entr(float ***stars,float **spos,int npc,int Nstar0,int Ng0,int Ng,
  float ***PCs,float ***coeff,float gain,int *Nobj,float snrs,float ***wgt){
	...
	int Nstar,nbond=(Ng0-Ng)/2+1;
	float ***star0,**start,**starf,**residu,***star,***residual;
  ...
  residual=f3tensor(0,Nstar0,0,Ng0,0,Ng0);
	...
	compcoeff=dmatrix(0,Nstar0,0,npc);
  ...
  *Nobj=Nstar;
  ...
  for(ic=0;ic<Nstar;ic++){
    ...
    for(i=0;i<Ng;i++)for(j=0;j<Ng;j++){
      ...
      //star0[ic][i][j]=starf[i][j];
      //stars[ic][i][j]=a[ic][i*Ng+j];
      //residual[ic][i][j]=weight[ic][i*Ng+j];
      //residual[ic][i][j]=starf[i][j];
    }
  ...
  EMPCA(a,Nstar,Mp,compbfr,compcoeff,coefferr,weight,npc);
    ...
    for(ic=0;ic<Nstar;ic++){
      for(ipc=0;ipc<npc;ipc++){
        ...
        sumt=fabs(compcoeff[ic][ipc]/sum);
        if(sumt<1.5)compcoeff[ic][ipc]=0.;
      }
    }printf("Coeff down\n");

    for(ic=0;ic<Nstar;ic++)for(k=0;k<npc;k++){
    	coeff[ic][k][0]=compcoeff[ic][k];
    ...
    for(ic=0;ic<Nstar;ic++){
      ...
      for(i=0;i<Ng;i++)for(j=0;j<Ng;j++){
        start[i][j]=0;
        for(ipc=0;ipc<npc;ipc++){
          start[i][j]+=compbfr[ipc][i*Ng+j]*compcoeff[ic][ipc];
        }
    ...
    for(ic=0;ic<Nstar;ic++){
      ...
      if(cmpara[1]<=0.05 && cmpara[2]<=0.05 && cmpara[0]<=0.1){
        ...
        for(i=0;i<Ng0;i++)for(j=0;j<Ng0;j++)stars[count][i][j]=stars[ic][i][j];
        count++;
      }
    }
    count=Nstar;
