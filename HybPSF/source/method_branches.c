...
void web_method_branches(float ***stars,float **spos,int npc,int Nstar,int Ng0,int Ng,
  ...
  switch(method){
    case(1):webiSPCA_entr(stars,spos,npc,Nstar,Ng0,Ng,PCs,coeff,gain,Nobj,snrs,osam,imdoel,wgt,wcoff,big);break;
...
void web_psf_fit(float *get_stars,float *get_spos,int npc,int Nstar,int Ng0,int Ng,
  ...
  float snrs,float *imodel,float *wgt,float *get_wcoff,int big){
    ...
    web_method_branches(stars,spos,npc,Nstar,Ng0,Ng,PCs,
      coeff,gain,method,&Nobj,osam,snrs,models,mask,wcoff,big);
    ...
    printf("out method_branches.c: web_psf_fit\n");
