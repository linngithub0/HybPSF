...
/* allocate a float 4tensor with range t[nrl..nrh][ncl..nch][ndl..ndh][nwl..nwh] */
{
	...
	for(j=ncl+1;j<=nch;j++)t[nrl][j]=t[nrl][j-1]+ndep;
	for(k=ndl+1;k<=ndh;k++)t[nrl][ncl][k] = t[nrl][ncl][k-1] + nwid ;
	for(i=nrl+1;i<=nrh;i++){
		t[i] = t[i-1] + ncol ;
		t[i][ncl]=t[i-1][ncl]+ncol*ndep;
		t[i][ncl][ndl] = t[i-1][ncl][ndl] + ncol*ndep*nwid ;
		
		for(k=ndl+1;k<=ndh;k++)t[i][ncl][k] = t[i][ncl][k-1] + nwid ;
		for(j=ncl+1;j<=nch;j++){
			t[i][j]=t[i][j-1] + ndep ;
			t[i][j][ndl] = t[i][j-1][ndl] + ndep*nwid ;
			for(k=ndl+1;k<=ndh;k++)t[i][j][k] = t[i][j][k-1] + nwid ;
			
		}
	}
	for(j=ncl+1;j<=nch;j++){
	  t[nrl][j][ndl]=t[nrl][j-1][ndl]+ndep*nwid;
	  for(k=ndl+1;k<=ndh;k++)t[nrl][j][k] = t[nrl][j][k-1] + nwid ;
	}
...
void free_d4tensor(double ****t, long nrl, long nrh, long ncl, long nch, 
	long ndl, long ndh, long nwl, long nwh)
/* free a float d4tensor allocated by f4tensor() */
{
	free((FREE_ARG) (t[nrl][ncl][ndl]+nwl-NR_END));
	free((FREE_ARG) (t[nrl][ncl]+ndl-NR_END));
	free((FREE_ARG) (t[nrl]+ncl-NR_END));
...
float ****f4tensor(nch,ncl,ndh,ndl,nrh,nrl,nwl,nwh)
  long nch,ncl,ndh,ndl,nrh,nrl,nwl,nwh;
/* allocate a float 4tensor with range t[nrl..nrh][ncl..nch][ndl..ndh][nwl..nwh] */
{
	long i,j,k, nrow=nrh-nrl+1,ncol=nch-ncl+1,ndep=ndh-ndl+1,nwid=nwh-nwl+1 ;
	float ****t;
...
double ****d4tensor(nch,ncl,ndh,ndl,nrh,nrl,nwl,nwh)
  long nch,ncl,ndh,ndl,nrh,nrl,nwl,nwh;
/* allocate a double 4tensor with range t[nrl..nrh][ncl..nch][ndl..ndh][nwl..nwh] */
{
	long i,j,k, nrow=nrh-nrl+1,ncol=nch-ncl+1,ndep=ndh-ndl+1,nwid=nwh-nwl+1 ;
...
void free_f4tensor(t,nrl,nrh,ncl,nch,ndl,ndh,nwl,nwh)
float ***t;
long nch,ncl,ndh,ndl,nrh,nrl;
/* free a float f4tensor allocated by f3tensor() */
{       
        free((FREE_ARG) (t[nrl][ncl][ndl]+nwl-NR_END));
	free((FREE_ARG) (t[nrl][ncl]+ndl-NR_END));
	free((FREE_ARG) (t[nrl]+ncl-NR_END));
...
void free_d4tensor(t,nrl,nrh,ncl,nch,ndl,ndh,nwl,nwh)
double ***t;
long nch,ncl,ndh,ndl,nrh,nrl;
/* free a float d4tensor allocated by f3tensor() */
{       
        free((FREE_ARG) (t[nrl][ncl][ndl]+nwl-NR_END));
	free((FREE_ARG) (t[nrl][ncl]+ndl-NR_END));
	free((FREE_ARG) (t[nrl]+ncl-NR_END));
