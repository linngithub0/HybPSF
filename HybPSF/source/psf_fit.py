...
def get_webPSF(Cam,
    ...
    Cam,            the camera of JWST, now only "NIRCam" are valid
    Filter,         the filter of the declared camera, such as F090W
    detector,       the detailed detector of camera,such as nrca*, nrcb*, nrcalong*, nrcalong*, 
                    and "*" are the number of: 1,2,3,4
    expos,          appoint which exposure of data, now can be 1,2,3,4,5,6,7,8,9
    ...
    '''
    import webbpsf
    fdirs="/Users/linn/Documents/data/JWST/"+Cam+"/"+Filter+"/"+detector+"/"
    fdirs="data/PC/JWST/"+Cam+"/"+Filter+"/"+detector+"/"
    print(fdirs)
    indx=str(int(expos/10000))+str(int(expos/1000)%10)+str(int(expos/100)%10)+str(int(expos/10)%10)+str(expos%10)
    append="_"+indx+"_"+detector+"_cal_pv181_rev4_PC.fits"
    
    if len(psfpos.shape)==1: 
        Nobj=1;psfcat=np.zeros((Nobj,2));
        psfcat[0][0]=psfpos[0];psfcat[0][1]=psfpos[1]
    else : 
        Nobj=psfpos.shape[0];psfcat=np.zeros((Nobj,2));
        for ic in range(Nobj):
            psfcat[ic][0]=psfpos[ic][0];psfcat[ic][1]=psfpos[ic][1];
    rPSF=np.zeros((Nobj,Target_Ng*osam,Target_Ng*osam),dtype='float')
    get_name=get_filename(fdirs,append);print(get_name)
    Tmodel=np.zeros((Nobj,Target_Ng*osam,Target_Ng*osam),dtype='float')
    print(Tmodel.shape)
    if len(get_name)==0: #use only webbpsf
        if Cam=="NIRCam": webp=webbpsf.NIRCam()
        if Cam=="MIRI": webp=webbpsf.MIRI()
        webp.filter=Filter;webp.detector=detector.upper()
        for ic in range(Nobj):
            webp.detector_position=(psfcat[ic][0],psfcat[ic][1])
            psf = webp.calc_psf(fov_pixels=Target_Ng*osam,oversample=osam)
            psf=psf[0].data.T;psf/=np.sum(psf)
            Tmodel[ic][:,:]=psf[:,:]

    else : # use mixture psf model
        hdu=fits.open(fdirs+get_name[0])
        PCs=hdu[0].data
        npc=PCs.shape[0]
        spos=hdu[1].data
        Nstar=spos.shape[0]
        coeff=hdu[2].data
        if Nstar<9: polyorder=1 
        else : polyorder=2
        residu=web_psf_rec(psfcat,PCs,spos,coeff,polyorder,method='poly')
        eNg=residu.shape[1]
        for ic in range(Nobj):
            print("ic in range(Nobj)",ic)
            for i in range(eNg):
                for j in range(eNg):
                    if np.fabs(residu[ic][i][j])<10**(-4):residu[ic][i][j]=0
                    #exclude the weak constarin region
        if Target_Ng<eNg/2:
            ...
            if Cam=="MIRI": webp=webbpsf.MIRI()
            webp.filter=Filter;webp.detector=detector.upper()
            dpix=int((eNg-Target_Ng*2)/2)
            for ic in range(Nobj):
                webp.detector_position=(psfcat[ic][0],psfcat[ic][1])
                psf = webp.calc_psf(fov_pixels=eNg/2,oversample=2)
                psf=psf[0].data.T;psf/=np.sum(psf)
                tmppsf=psf[:,:]+residu[ic][:,:]
                #tmppsf=residu[ic][:,:]
                ...
                #plt.imshow(np.sqrt(tmppsf));plt.show()
                if osam==2:
                  cent[0]=(Target_Ng/2.+0.5)*2.+1
                  cent[1]=(Target_Ng/2.+0.5)*2.+1
                  #tmp=BInterp_bicubic(tmppsf,cent,Target_Ng*2,Target_Ng*2)
                  tmp=tmppsf[dpix:dpix+Target_Ng*2,dpix:dpix+Target_Ng*2]
                  Tmodel[ic][:,:]=tmp[:,:]
                if osam==1:#merger pixels
                  print("case 1")
                  cent[0]=(Target_Ng/2.+0.5)*2.+1
                  cent[1]=(Target_Ng/2.+0.5)*2.+1
                  #tmp=BInterp_bicubic(tmppsf,cent,Target_Ng*2,Target_Ng*2)
                  tmp=tmppsf[dpix:dpix+Target_Ng*2,dpix:dpix+Target_Ng*2]
                  #plt.imshow(np.sqrt(tmp));plt.show()
                  for i in range(Target_Ng):
                    for j in range(Target_Ng):
                      for l in range(2):
                        for m in range(2):
            ...
            dpix=int((Target_Ng*2-eNg)/2);print("case 2")
            if Cam=="NIRCam": webp=webbpsf.NIRCam()
            if Cam=="MIRI": webp=webbpsf.MIRI()
            webp.filter=Filter;webp.detector=detector.upper()
            for ic in range(Nobj):
              webp.detector_position=(psfcat[ic][0],psfcat[ic][1]) 
              psf = webp.calc_psf(fov_pixels=Target_Ng,oversample=2)
              psf=psf[0].data.T;psf/=np.sum(psf)
              ratio=1./np.sum(psf[dpix:dpix+eNg,dpix:dpix+eNg]);
              psf*=ratio; #calibrate the flux ratio
              psf[dpix:dpix+eNg,dpix:dpix+eNg]+=residu[ic][:,:]
              tmp=psf
              for i in range(eNg):
                    for j in range(eNg):
                        if  tmp[i][j]<0 : #padding the nagetive value
                            tmp[i][j]=0
              for i in range(Target_Ng*2):
                for j in range(Target_Ng*2):
                  if  tmp[i][j]==0 : #padding the nagetive value
                    tmp[i][j]=(tmp[i-1][j-1]+tmp[i-1][j]+tmp[i-1][j+1]+
                    tmp[i][j-1]+tmp[i][j+1]+
                    tmp[i+1][j-1]+tmp[i+1][j]+tmp[i+1][j+1])/8.
              tmp/=np.sum(tmp)
              if osam==2:
                Tmodel[ic][:,:]=tmp[:,:]
              if osam==1:#merger pixels
