...
#include "nrutil.h"
#include "fitsio.h"
int staridf(int argc,char *argv[])
{
  ...
  if(argc!=8 && argc!=6 && argc!=9 && argc!=10){
    ...
    printf("or :%s <raw cata file path> <input cata name> <fits path> <input fitsname> <output path> \
    <pixel size> [<left> <right>] [<gain>] [<weight fits>]\n",
      argv[0]);
    exit(EXIT_FAILURE);
  }
  ...
  printf("%s\t%s\t%s\t%s\t%s\t%s\n",flist0,fname0,fnames,fnameln,fnameln,fbox );
    ...
    char comment[100],comment_DATE[100],DATE[100]={"DATE"},value_DATE[100],stmp[100];    
  ...
  if(argc==7){
    ...
    minx=boxf[0][0]=box50[0][0]=atof(argv[7]);maxx=boxf[1][0]=box50[1][0]=atof(argv[8]);//left & right
  }
  ...
  for(i=0;i<Nstar;i++){
    ...
    x--;y--;
    for(j=0;j<Nstar;j++){
      x1=fabs(pos[j][0]-x)*1.5;y1=fabs(pos[j][1]-y)*1.5;
  ...
  Nstar=m;
  if(Nstar==0){
    printf("No star's snr is lager than 40, press Enter and exit");
