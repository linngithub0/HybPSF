		 ...
		 double *para,double *gamma)
{
  ...
  int i,j,k;
  double x,y,sum,scale,rt2,sigmaw=3.2,fwhm,rd,beta,es[2],moments[5];
     ...
     double *para,double *gamma)
{
  ...
  int i,j,k;
  double x,y,sum,scale,rt2,sigmaw=3,fwhm,rd,beta,es[2],moments[5];
  double weight,xc,yc,g1,g2,gscale,rd2v,r20,r42;
  g1=0;
  g2=0;
  gscale=1./(1.-g1*g1-g2*g2);
  ...
  for(i=0;i<100;i++){
    ...
    r20=moments[1]/moments[0];
    beta=1/((moments[2]*moments[0])/(2*moments[1]*moments[1])-1);
    if(beta<1){beta=3.5;}if(beta>100){beta=100;}
