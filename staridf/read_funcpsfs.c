...
void funcpsfs_read(char *fname,int Nf,int khead,float **para,int npara,char *delim){ 
  ...
  char ftmp[500]={"#"},sget[500];
  char comp[2]={"#"};
  FILE *fp;
  int k,i,j;
  long iseed;
  fp=fopen(fname,"r");
  for(k=0;k<khead;k++)(fgets(ftmp,500,fp));//printf("Nf=%d\n",Nf);
  for(k=0;k<Nf;k++){
    if(fgets(ftmp,500,fp)!=NULL){
      //printf("%s\n",ftmp );
      for(i=0;i<npara;i++){
        if(i==0){
          strcpy(sget,strtok(ftmp,delim));
          para[k][i]=atof(sget);//printf("%f ", para[k][i]);
        }
        else{
          strcpy(sget,strtok(NULL,delim));
          para[k][i]=atof(sget);//printf("%f ", para[k][i]);
        }
      }//printf("\n");
  ...
  /*for(k=0;k<Nf;k++){//printf("k=%d\n",k );
    i=0; //index of para
    if(fgets(ftmp,500,fp)!=NULL){printf("%s\n",ftmp );
      j=0;*/
      //while(ftmp[j]!='\0'){printf("%c",ftmp[j] );j++;
        /*j=0; //index of ftmp
        l=0; //index of delim
        m=0; //index of sget
        if(ftmp[j]!=*delim){
          if(m==0 && j!=0){
            for(n=0;n<50;n++){sget[n]='\0';}
            para[k][i]=atof(sget);
            i++;
          }
          sget[m]=ftmp[j];
